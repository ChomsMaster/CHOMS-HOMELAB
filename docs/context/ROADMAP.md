...
| Scrutiny server privilege reduction | `pending` | The sensitive cold-bootstrap copy and isolated restore are validated. Before changing privilege or host paths, resolve controlled InfluxDB operator-token recovery and implement the separate official recurring backup needed for RPO 24h; then narrow only proven-unnecessary access and validate UI plus collector ingestion. |
