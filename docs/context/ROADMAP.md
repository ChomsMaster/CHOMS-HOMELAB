...
| Scrutiny server privilege reduction | `pending` | The sensitive cold-bootstrap copy, isolated restore and dedicated InfluxDB operator-token recovery are validated. Before changing privilege or host paths, implement and restore-test the separate official recurring backup needed for RPO 24h; then narrow only proven-unnecessary access and validate UI plus collector ingestion. |
