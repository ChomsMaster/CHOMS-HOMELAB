...
| Scrutiny server privilege reduction | `pending` | First implement and validate the separately designed Scrutiny SQLite/InfluxDB backup and isolated restore (RPO 24h, RTO 1–2h); only then narrow host paths and privilege, add justified CPU/startup controls, and validate UI plus collector ingestion. |
