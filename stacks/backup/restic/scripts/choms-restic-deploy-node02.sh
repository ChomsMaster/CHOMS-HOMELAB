...
NODE=choms-node-02
KEY=/root/.ssh/choms-restic-node02
...
FINALIZER=choms-restic-deploy-finalize-node02.service
TIMER=choms-restic-backup-node02.timer
SFTP_COMMAND='ssh -i /root/.ssh/choms-restic-node02 -o BatchMode=yes -o NumberOfPasswordPrompts=0 -o PasswordAuthentication=no -o KbdInteractiveAuthentication=no -o IdentitiesOnly=yes -o IdentityAgent=none -o StrictHostKeyChecking=yes -o UserKnownHostsFile=/root/.ssh/choms-restic-known_hosts choms-restic@192.168.1.134 -s sftp'
...
install_execute_support() {
  ...
  bundle_install "$SELF_DIR/$(basename "${BASH_SOURCE[0]}")" /usr/local/sbin/choms-restic-deploy-node02.sh 0755
...
prepare() {
  ...
  install -o chomsmaster -g chomsmaster -m 0600 "${KEY}.pub" "$output/node02.pub"
...
execute() {
  ...
  if snapshot_present choms-node-02 type-jellyfin; then
...
case "${1:-}" in
  ...
  finalize) finalize ;;
  validate) validate ;;
  *) bundle_fail 'usage: choms-restic-deploy-node02.sh {prepare|execute|finalize|validate}' ;;
esac
